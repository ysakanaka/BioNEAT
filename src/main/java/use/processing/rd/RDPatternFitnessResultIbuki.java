...
// fitness=distanceTopology(pattern,positions);
 ...
 public static double distanceNicolasExponential(boolean[][] pattern,boolean[][] positions){
	  ...
	  for(int distance=1;distance<farthestDistance;distance++){
       ...
       double rdistance = (double) distance / Math.sqrt(RDConstants.hsize * RDConstants.hsize + RDConstants.wsize * RDConstants.wsize);
	   fitness+=fitnessTemp*RDConstants.matchPenalty*Math.exp(weightExponential*rdistance*RDConstants.spaceStep/2.0); //matchPenalty < 0
	  }
	  return fitness;
}
...
// drawBlurredPattern(pattern);
 ...
 public static boolean[][] getTPattern(){ // Centerline + topline
  boolean[][] pattern=new boolean[(int)(RDConstants.wsize/RDConstants.spaceStep)][(int)(RDConstants.hsize/RDConstants.spaceStep)];
  for(int i=0;i<pattern.length;i++){
   for(int j=0;j<pattern[i].length;j++){
    pattern[i][j]=(i>=pattern.length*(0.5f-width/2.0f)&&i<=pattern.length*(0.5f+width/2.0f)) | (j<=pattern[0].length*width);
   }
  }
  return pattern;
 }
 ...
 public static boolean[][] getKoKanji(){ // Centerline + topline + Bottomline
  ...
  for(int i=0;i<pattern.length;i++){
   for(int j=0;j<pattern[i].length;j++){
    pattern[i][j]=(i>=pattern.length*(0.5f-width/2.0f)&&i<=pattern.length*(0.5f+width/2.0f)) | (j<=pattern[0].length*width) | (j>=pattern[0].length*(1.-width));
   }
  }
  return pattern;
 }
