...
public class TestStepFitness {
	public static void main(String[] args) throws FileNotFoundException, InterruptedException, ExecutionException{
		...
		StepFitnessFunction gf = new StepFitnessFunction();
		StepFitnessFunction.hysteresis = false;
		StepFitnessFunction.diff = false;
		
		gf.nTests = 101;
		
		if(StepFitnessFunction.hysteresis) gf.nTests *= 2;
		
		AbstractMathFitnessFunction.setSaveSimulation(true);
		OligoSystemComplex.selfStart = true;
		FitnessResult fitness = (FitnessResult) gf.evaluate(rn);
		System.out.println(fitness);
		System.out.println(Arrays.toString(fitness.targetFittingParams));
		System.out.println(Arrays.toString(fitness.actualFittingParams));
		System.out.println(Arrays.toString(fitness.targetOutputs));
		window.displayEvaluationResults(rn, fitness,new StepFitnessDisplayer());
	}
}
