 ...
 * @param <E>
 */
public class TemplateEqWriter<E> {
	...
	public TemplateEqWriter(OligoGraph<SequenceVertex,E> g, E t){
		this.graph = g;
		this.template = t;
	...
	public TemplateEqWriter(OligoGraph<SequenceVertex,E> g, E t, boolean numerical){
		this.graph = g;
		this.template = t;
		inhib = graph.getInhibition(t) != null ? graph.getInhibition(t).getLeft(): null;
		from = graph.getSource(t);
	...
	public static void main(String[] args){
		...
		int baseIndex = 1;
		TemplateEqWriter<String> t = new TemplateEqWriter<String>(g,edge);
		...
		eqs = (new TemplateEqWriter<String>(g,edge,false)).getEqs(baseIndex);
		for (int i = 0; i< eqs.length; i++){
			System.out.println("delta_"+Utils.idToString(baseIndex+i)+" = "+eqs[i]+";");
		}
		System.out.println();
		System.out.println("Test 4\n===========================");
		eqs = (new TemplateEqWriter<String>(GraphMaker.makeOligator(),edge,false)).getEqs(baseIndex);
		for (int i = 0; i< eqs.length; i++){
			System.out.println("delta_"+Utils.idToString(baseIndex+i)+" = "+eqs[i]+";");
		}
	}
